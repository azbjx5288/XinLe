...
import com.xinle.lottery.data.BackToken;
import com.xinle.lottery.data.BindCardDetail;
import com.xinle.lottery.data.CardDetailCommand;
import com.xinle.lottery.data.ReviseCommand;
import com.xinle.lottery.data.UserInfo;
import com.xinle.lottery.data.UserInfoCommand;
import com.xinle.lottery.data.WithdrawCommand;
...
public class DrawFragment extends BaseFragment {
    ...
    private void loadRevise(String pass, int type) {
        ...
        if (cardDetails.size() > 0) {
            ...
            command.setAccount_name(cardDetails.get(0).getAccountName());
    ...
    private void submit() {
        ...
        try {
            ...
            command.set_bankcardId(String.valueOf(checkCard.getId()));
            command.set_fundPassword(fundPassword.getText().toString());//DigestUtils.md5Hex(DigestUtils.md5Hex(DigestUtils.md5Hex(fundPassword.getText().toString())))
            command.set_amount(drawMoneyEditText.getText().toString());
    ...
    private View.OnClickListener onClickCardListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            RecordTime withdrawaPhase = ConstantInformation.getLasttime(checkCard.getModifiedAt(), ConstantInformation.df.format(new Date()));
            if (!checkCard.isLocked() && withdrawaPhase.getDay() == 0 && withdrawaPhase.getHour() < 2) {
                submit.setVisibility(View.GONE);
                withdrawalsTime.setVisibility(View.VISIBLE);
                RecordTime lockedTime = ConstantInformation.getLasttime(checkCard.getModifiedAt(), checkCard.getIntendedWithdrawalAt());
                withdrawalsTime.start(ConstantInformation.ONE_HOUR * lockedTime.getHour() + ConstantInformation.ONE_MINUTE * lockedTime.getMinute() + 1000L * lockedTime.getSecond());
