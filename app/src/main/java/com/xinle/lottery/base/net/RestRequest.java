package com.xinle.lottery.base.net;
...
public class RestRequest implements Response.ErrorListener, Response.Listener<RestResponse> {
    ...
    public GsonRequest createGsonRequest(Type type) {
        ...
        request.setRetryPolicy(new DefaultRetryPolicy((int)(3 * ONE_SECOND), 3, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        return request;
    }
    ...
    public void execute() {
        ...
        if (XinLeApp.getNetStateHelper().isConnected()) {
...
//        handleErrorDefault();
    ...
    private boolean handleNotLogin() {
        ...
        LoginCommand command = XinLeApp.getUserCentre().createLoginCommand();
        if (command == null) {
            //不能自动登录
            return false;
        }
        ...
        RestRequestManager.executeCommand(context, command, new RestCallback() {
            ...
            public boolean onRestComplete(RestRequest request, RestResponse response) {
                ...
                RestRequest.this.request.setRetryPolicy(new DefaultRetryPolicy((int)(3 * ONE_SECOND), 3, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
                execute();
                return false;
            }
