...
public final class GsonRequest extends Request<RestResponse> {
    ...
    public GsonRequest(int method, RestRequest request) {
        ...
        cacheKey = BuildConfig.VERSION_CODE
                + XinLeApp.getUserCentre().getUserID()
    ...
    private static String convertUrl(int method, RestRequest request) {
        if (method != Method.GET) {
            return XinLeApp.getUserCentre().getUrl(request.getConfig().api());
        }

        String baseUrl = XinLeApp.getUserCentre().getUrl(request.getConfig().api());
        ...
        try {
            ...
            if(encoded!=null&&encoded.length()>0){
                ...
                if(sp.length>=1){
                    ...
                    encodedParams.append("sign="+DigestUtils.md5Hex(URLEncoder.encode(sign, "UTF-8")+ XinLeApp.getUserCentre().getKeyApiKey()));
                }
            }
    ...
    public Map<String, String> getHeaders() throws AuthFailureError {
        ...
        String cookie = XinLeApp.getUserCentre().getSession();
        if (cookie != null) {
